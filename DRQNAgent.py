...
class TransformerQ_net(nn.Module):
    ...
    def __init__(self, state_space, action_space, hidden_size, num_agents=200, embed_dim=64,
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            dropout=0.05, activation='relu', batch_first=True, norm_first=True)
        ...
        # GTrXL-P0: 门控残差, cross-sat 分支近零 init + 近闭 gate (sigmoid(-0.85)≈0.30)
        for m in self.cross_heads:
            nn.init.normal_(m.weight, std=1e-3)
            nn.init.zeros_(m.bias)
        self.gate = nn.Parameter(torch.tensor(-0.85))
        self.v_heads = nn.ModuleList([nn.Linear(16, 1) for _ in range(num_agents)])
        self.a_heads = nn.ModuleList([nn.Linear(16 + 16, 1) for _ in range(num_agents)])
        self._head_idx = 0
        # GTrXL-P0: 逐卫星 token 的共享时序 GRU (开关 LEO_TF_TEMPORAL, 默认开)
        self.temporal = os.environ.get('LEO_TF_TEMPORAL', '1') == '1'
        if self.temporal:
    ...
    def forward(self, input, h, c, head_indices=None):
        ...
        F = self.feat_per_sat
        H = self.hidden_size
        gate_s = torch.sigmoid(self.gate)   # GTrXL-P0 门控残差
        ...
        # GTrXL-P0: 逐卫星 token 的共享时序编码 (跨星 attention 之前)
        if self.temporal:
            sat_h_t = sat_h.permute(0, 2, 1, 3).reshape(B * K, seq_len, H)  # [B*K, seq, H]
            sat_h_t, _ = self.temporal_gru(sat_h_t)
            sat_h = sat_h_t.reshape(B, K, seq_len, H).permute(0, 2, 1, 3)   # [B, seq, K, H]

        # Flatten: [B*seq, K, H] → Transformer跨卫星attention
        tokens = sat_h.reshape(B * seq_len, K, H)
        ...
        if head_indices is not None:
            for idx in range(self.num_agents):
                ...
                if m.any():
                    ...
                    sat_feat[m] = raw_feat + gate_s * cross_feat
        else:
            ...
            sat_feat = raw_feat + gate_s * cross_feat
...
class LSTMAttnQ_net(nn.Module):
    ...
    def __init__(self, state_space, action_space, hidden_size, num_agents=200, embed_dim=64,
        ...
        self.sat_nets = nn.ModuleList([nn.Sequential(nn.Linear(feat_per_sat, 32), nn.ReLU(),
                                       ...
                                       for _ in range(num_agents)])
        self.cross_heads = nn.ModuleList([nn.Linear(hidden_size, 16) for _ in range(num_agents)])
        for m in self.cross_heads:                 # 近零 init，初始模型≈纯 LSTM
            nn.init.normal_(m.weight, std=1e-3)
            nn.init.zeros_(m.bias)
        self.gate = nn.Parameter(torch.tensor(-0.85))   # sigmoid(-0.85)≈0.30，梯度健康
        self.use_gate = os.environ.get('LEO_ATTN_GATE', '0') == '1'
        self.v_heads = nn.ModuleList([nn.Linear(16, 1) for _ in range(num_agents)])
        self.a_heads = nn.ModuleList([nn.Linear(16 + 16, 1) for _ in range(num_agents)])
        self._head_idx = 0

    def forward(self, input, h, c, head_indices=None):
        gate = torch.sigmoid(self.gate) if self.use_gate else 1.0
        B, seq_len = input.shape[0], input.shape[1]
        K = self.top_k
        F = self.feat_per_sat
        H = self.hidden_size
        ...
        if head_indices is not None:
            for idx in range(self.num_agents):
                ...
                if m.any():
                    ...
                    sat_feat[m] = raw_feat + gate * cross_feat
        else:
            ...
            sat_feat = raw_feat + gate * cross_feat
...
class UserAgent:
    def __init__(self, user:User, env:Handover, c_agent, gamma=0.9, epsilon=0.01, batch=256, buffer=2000, hidden_size=128, seq=6, device='auto', head_idx=0):
        ...
        use_lstm_attn = os.environ.get('LEO_LSTM_ATTN', '0') == '1'
        if use_lstm_attn:
    ...
    def step(self,observation,reward, h, c):
        ...
        self._last_n_valid = n_valid  # 供CSV诊断列
        ...
        if (last_ho is not None
            ...
            and self.env.topo.current_time - last_ho < MIN_HOLD_SLOTS * SLOT_SECONDS):
...
class CenterAgent:
    def __init__(self, env:Handover, gamma=0.9, epsilon=0.01, batch=256, buffer=5000, hidden_size=128, lr=0.001, seq=6, device='auto'):
        ...
        if use_lstm_attn:
            ...
            self.evaluate_net = Q_net(state_dim + 64, K, hidden_size, num_agents=n_agents, top_k=K).to(self.device)
        # ── 手段A: 冻结 raw 路径 (LEO_FREEZE_RAW=1 + LEO_RAW_CKPT), 只训 attention 分支 ──
        if os.environ.get('LEO_FREEZE_RAW', '0') == '1':
            raw_ckpt = os.environ.get('LEO_RAW_CKPT', '')
            ck = torch.load(raw_ckpt, map_location='cpu', weights_only=False)
            raw_state = ck['evaluate_net'] if 'evaluate_net' in ck else ck
            self.evaluate_net.load_state_dict(raw_state, strict=False)
            _freeze_prefix = ('agent_embed', 'lstm', 'ctx_fcs', 'sat_nets', 'v_heads', 'a_heads')
            for _n, _p in self.evaluate_net.named_parameters():
                if _n.startswith(_freeze_prefix):
                    _p.requires_grad = False
            print('  [CenterAgent] freeze-raw 加载 %s' % raw_ckpt, flush=True)
        self.optimizer = optim.Adam(filter(lambda p: p.requires_grad, self.evaluate_net.parameters()), lr=lr)
...
def save_checkpoint(c_agent, u_agents, episode, path):
    ckpt = {
        'evaluate_net': c_agent.evaluate_net.state_dict(),
        'target_net': c_agent.target_net.state_dict(),
        'optimizer': c_agent.optimizer.state_dict(),
        'episode': episode,
        'epsilons': [a.epsilon for a in u_agents],
        'user_memory': {a.head_idx: list(a.replayer.memory) for a in u_agents},
        'center_memory': {a.head_idx: list(c_agent.replayer.memory[a]) for a in u_agents},
        'center_priorities': {a.head_idx: list(c_agent.replayer.priorities[a]) for a in u_agents},
    }
    torch.save(ckpt, path)


def train_episode(env:Handover, u_agents, c_agent, model, mode=None,start_time=0, end_time=150000, time_step=SLOT_SECONDS, net_step=20,batch=256, patience=5, min_episodes=2000, conv_threshold=0.005):
    """训练一个DRQN episode序列，支持早停。

    Args:
        patience: 连续N个统计窗口无改善即早停 (每个窗口=100 episodes)
    ...
    episode = getattr(c_agent, 'start_episode', 0)   #训练次数 (resume续跑)
    actions = {}        #每个时隙内的agent动作集
    env.reset(time,'NETWORK_LOAD')  #
    c_agent.reset(u_agents)
    # resume: 灌回回放池 + 精确恢复 target_net
    _resume = getattr(c_agent, '_resume_ckpt', None)
    if _resume is not None:
        c_agent.target_net.load_state_dict(_resume['target_net'])
        for agent in u_agents:
            h = agent.head_idx
            for t in _resume.get('user_memory', {}).get(h, []):
                agent.replayer.memory.append(t)
            for t in _resume.get('center_memory', {}).get(h, []):
                c_agent.replayer.memory[agent].append(t)
            for pr in _resume.get('center_priorities', {}).get(h, []):
                c_agent.replayer.priorities[agent].append(pr)
        c_agent._resume_ckpt = None
    ...
    while(time<=end_time):
        ...
        # 最新loss
        loss_val = getattr(c_agent, 'last_loss', 0.0)
        ep_n_valid = sum(getattr(a, '_last_n_valid', 0) for a in u_agents) / len(u_agents)
        per_ep_log.write(f'{episode},{ep_total:.4f},{ep_rate:.0f},{ep_hops:.1f},{ep_ho:.3f},'
                         ...
                         f'{ep_q_gap_avg:.4f},{ep_q_gap_noise:.3f},{ep_n_valid:.1f}\n')
        per_ep_log.flush()
        # 每50ep存档，防中断丢失
        if episode % 50 == 0:
            save_checkpoint(c_agent, u_agents, episode, './log/model/%s_ep%d.pkl' % (model, episode))
    ...
    save_checkpoint(c_agent, u_agents, episode, './log/model/%s.pkl'%model)
    writer.close()
    env.close()
    reward_list[0] = -1
    reward_list[1] = -1
