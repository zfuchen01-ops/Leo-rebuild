...
def make_user_locations(count: int, seed: int = 42):
    """生成随机用户位置, 返回 [(lon_rad, lat_rad), ...]."""
    import random
    rng = random.Random(seed)
    users = []
    for _ in range(count):
        lat = rng.uniform(-60, 60) * _pi / 180.0  # 与v1.1/DQN一致
        lon = rng.uniform(-120, 120) * _pi / 180.0
        users.append((lon, lat))
    return users


def build_drqn_env(user_count: int, constellation: str, seed: int = 42):
    ...
    # 添加用户
    user_locs = make_user_locations(user_count, seed)
    ...
    for user in topo.user:
        user.assigned_gateway = topo.gateway[rng.randrange(len(topo.gateway))]
...
def main():
    ...
    import DRQNAgent
    import os as _os
    # 防覆盖: 从头跑时若同 tag 输出已存在则自动换 tag; resume 时 CSV 追加续写
    _csv_path = f'./log/RL/DRQN_{args.tag}_per_ep.csv'
    _final_model = f'./log/model/drqn_{args.constellation}_u{args.users}_{args.tag}.pkl'
    if args.resume:
        _csv_mode = 'a'; _write_header = False
    else:
        _csv_mode = 'w'; _write_header = True
        if _os.path.exists(_csv_path) or _os.path.exists(_final_model):
            _n = 1
            while _os.path.exists(f'./log/RL/DRQN_{args.tag}_r{_n}_per_ep.csv') or \
                  _os.path.exists(f'./log/model/drqn_{args.constellation}_u{args.users}_{args.tag}_r{_n}.pkl'):
                _n += 1
            args.tag = f'{args.tag}_r{_n}'
            _csv_path = f'./log/RL/DRQN_{args.tag}_per_ep.csv'
            print(f'[warn] 同 tag 输出已存在, 自动改用 tag={args.tag} 避免覆盖历史数据', flush=True)
    DRQNAgent.per_ep_log = open(_csv_path, _csv_mode)
    if _write_header:
    ...
    print(f"  总时隙: {args.slots} (end_time={args.slots*SLOT_SECONDS}s)")
    print(f"  学习率: {args.lr}")
    print(f"  早停patience: {args.patience} 窗口 (={args.patience*100} ep)")
    ...
    if args.resume:
        ...
        if isinstance(ckpt, dict) and 'evaluate_net' in ckpt:
            c_agent.evaluate_net.load_state_dict(ckpt['evaluate_net'])
            c_agent.optimizer.load_state_dict(ckpt['optimizer'])
            resume_episode = int(ckpt.get('episode', 0))
            eps = ckpt.get('epsilons', [])
            for i, agent in enumerate(u_agents):
                if i < len(eps):
                    agent.epsilon = eps[i]
            c_agent._resume_ckpt = ckpt
        else:
            c_agent.evaluate_net = ckpt.to(c_agent.device)
        ...
        c_agent.start_episode = resume_episode
        print(f"  loaded ok, resume from episode {resume_episode}", flush=True)
    ...
    # 训练
    end_time_s = args.slots * SLOT_SECONDS
    print(f"开始训练... ({args.slots} 时隙, {end_time_s}s)")
    print("-" * 60)
    ...
    reward_list = train_episode(
        ...
        mode='train',
        start_time=resume_episode * SLOT_SECONDS,
        end_time=end_time_s,
        time_step=SLOT_SECONDS,
        net_step=20,
        batch=256,
